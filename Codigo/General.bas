...
If map > 0 And map < NumMaps + 1 And X > 0 And X <= XMaxMapSize And Y > 0 And Y <= YMaxMapSize Then
...
'Bordes del mapa
MinXBorder = 10 'XMinMapSize + (XWindow \ 2)
MaxXBorder = 1086 'XMaxMapSize - (XWindow \ 2)
MinYBorder = 15 'YMinMapSize + (YWindow \ 2)
MaxYBorder = 1491 'YMaxMapSize - (YWindow \ 2)
DoEvents
...
If UserList(UserIndex).Stats.MinSta < UserList(UserIndex).Stats.MaxSta Then
    If UserList(UserIndex).flags.Desnudo Then Intervalo = Intervalo * 6

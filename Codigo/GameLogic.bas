...
If (map <= 0 Or map > NumMaps) Or (map = 1 And (X < MinXBorder Or X > MaxXBorder Or Y < MinYBorder Or Y > MaxYBorder)) Then
    InMapBounds = False
ElseIf X < 1 Or X > XMaxMapSize Or Y < 1 Or Y > YMaxMapSize Then
    InMapBounds = False
Else
    InMapBounds = True
End If
...
End Function
Public Sub FindLegalPosNpc(ByVal map As Integer, ByRef X As Integer, ByRef Y As Integer, ByVal AguaValida As Byte, Optional ByVal IsPet As Boolean = False)
'***************************************************
'Autor: ZaMa
'Last Modification: 26/03/2009
...
'***************************************************
    ...
    If Not LegalPosNPC(map, X, Y, AguaValida, IsPet) Then
        ...
        For Rango = 1 To 8
            For tY = Y - Rango To Y + Rango
                For tX = X - Rango To X + Rango
                    ...
                    If LegalPosNPC(map, X, Y, AguaValida, IsPet) Then
